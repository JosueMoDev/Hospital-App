...
export class FileUploadMiddleware {
    ...
    static containFiles(request: Request, response: Response, next: NextFunction) {
        ...
        const isExtensionAllow = Object.values(AllowedExtensions).includes(fileExtension as AllowedExtensions);
        if (!isExtensionAllow) return response.status(400).json({ error: `Extension File must be included in ${Object.values(AllowedExtensions)}` })
        next();
    }
